...
for (let n = 0; n < stars.length; n++) {
    ...
    stars[n].position.set(randFloat(-15, 15), randFloat(-15, 15), randFloat(camera.position.z, camera.position.z - 150));
    scene.add(stars[n]);
}
...
earthLight.position.copy(earthOrbit.localToWorld(new THREE.Vector3(5 * Math.sin(earthShader.angle.value + earthShader.rotY.value), 15, 5 * Math.cos(earthShader.angle.value + earthShader.rotY.value))));
...
function solarSystem (time) {
    earthLight.position.copy(earthOrbit.localToWorld(new THREE.Vector3(15 * Math.sin(-earthShader.angle.value), 15, 15 * Math.cos(-earthShader.angle.value))));
...
function anim() {
    ...
    else if (btnActive.value === 1 && intersects[1] && intersects[1].object !== btnPressed) {
        buttonTl1.reverse();
    }
    ...
    if (flag) {
        ...
        if (camera.position.length() < space_length) {
            ...
            gsap.to(renderer.domElement.style, {opacity: 1, duration: 3}, '>3');
            setTimeout(() => {
        ...
        // else {
        //     let vect = new THREE.Vector3(7 * Math.sin(time / 4), 7 * Math.cos(time / 8), 7 * Math.cos(time / 4));
        //     camera.position.x = earthOrbit.position.x + vect.x;
        //     camera.position.y = earthOrbit.position.y - vect.y;
        //     camera.position.z = earthOrbit.position.z + vect.z;
        //     camera.lookAt(earthOrbit.position.x, earthOrbit.position.y, earthOrbit.position.z);
        //     camera.lookAt(camera.localToWorld(new THREE.Vector3(0.5, 0.3, -1)));
            // console.log(time)
            // console.log(camera.position)
            // controls.target = earthOrbit.position;
            // controls.update();
        // }
    }
...
function gsapInit() {
    let startTime = 14;
    gsap.to(".loading", {display: 'none', duration: 0}, startTime);
    gsap.to(renderer.domElement.style, {opacity: 1, duration: 1}, startTime);
    gsap.to(renderer.domElement.style, {display: 'block', duration: 0}, startTime);
    ...
    buttonTl2.to(btnPressed.position, {z: camera.position.z + 5, duration: 0.2}, 0.01)
            ...
            .to(matBack, {opacity: 1, duration: 0.5}, '<0')
